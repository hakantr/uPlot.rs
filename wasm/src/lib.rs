...
use uplot_rs_gpui_ornekler::{
    ...
    GPUI_SVG_EXPORT_KART_TANIM_ÖRNEĞİ as SVG_IMAGE_KART_TANIM_ÖRNEĞİ, GRADIENTS_KART_TANIM_ÖRNEĞİ,
    ...
    MONTHS_RU_KART_TANIM_ÖRNEĞİ, MULTI_BARS_KART_TANIM_ÖRNEĞİ, MultiBarsÖrneği,
    ...
    STACKED_SERIES_KART_TANIM_ÖRNEĞİ, STREAM_DATA_KART_TANIM_ÖRNEĞİ, SYNC_CURSOR_KART_TANIM_ÖRNEĞİ,
    SYNC_Y_ZERO_KART_TANIM_ÖRNEĞİ, ScalesDirOriÖrneği, ScatterÖrneği, SeriYaşamDöngüsüOlayı,
    SeçimEylemi, SineAkışı, SmoothingÖrneği, SoftMinMaxAkışı, SoftMinMaxÖrneği,
    ...
    dependent_scale_kartı, draw_hooks_kartı, focus_cursor_kartı,
    gpui_svg_export_kartı as svg_image_kartı, gradients_kartı, grid_over_series_kartı,
    ...
    scale_padding_kartı, scales_dir_ori_kartı, scatter_kartı, scroll_sync_kartı, soft_minmax_kartı,
    sparklines_bars_kartı, sparklines_kartı, sparse_kartı, stacked_series_kartı,
    stacked_series_kartı_görünür, stream_data_kartı, sync_cursor_kartı,
...
impl KartOturumu {
    ...
    pub fn zoom_ranger_surukleme_ekseni(&self, x_px: f64, y_px: f64) -> u8 {
        use uplot_rs_gpui_ornekler::ZoomRangerSürüklemeEkseni::{X, XY, Y, Yok};
    ...
    pub fn zoom_varyasyon_ekseni(
        ...
        use uplot_rs_gpui_ornekler::ZoomRangerSürüklemeEkseni::{X, XY, Y, Yok};
        let Some(kip) = uplot_rs_gpui_ornekler::ZoomSürüklemeKipi::TÜMÜ
            .get(kip_indeksi)
            .copied()
        else {
            return 0;
        ...
        self.grafik.zoom_ranger_durumu().map_or(0, |mut durum| {
            durum.sürükleme_ayarlarını_ayarla(
                uplot_rs_gpui_ornekler::ZoomRangerSeçenekleri::zoom_varyasyonu(
                    kip,
...
mod testler {
    ...
    fn scatter_wasm_iki_mode_iki_yüzeyini_ve_vuruşu_üretir() {
        ...
        let vuruş_adayı = oturum
            ...
            .find_map(|komut| match komut {
                uplot_rs::diagnostics::Komut::Daire { merkez, .. } => Some(*merkez),
                uplot_rs::diagnostics::Komut::DeğişkenDaireler { daireler, .. } => {
    ...
    fn soft_minmax_wasm_beş_yüzeyi_ve_canlı_artışı_korur() {
        assert_eq!(
            uplot_rs_gpui_ornekler::soft_minmax_kartları(12.0).map(|kartlar| kartlar.len()),
    ...
    fn sparklines_wasm_yirmi_kompakt_yüzeyi_korur() {
        let kartlar = uplot_rs_gpui_ornekler::sparklines_kartları();
    ...
    fn sparse_wasm_üç_kaynak_yüzeyini_korur() {
        let kartlar = uplot_rs_gpui_ornekler::sparse_kartları();
        assert!(kartlar.is_ok());
    ...
    fn stacked_series_wasm_on_altı_kaynak_yüzeyini_korur() {
        assert_eq!(
            uplot_rs_gpui_ornekler::stacked_series_kartları().map(|kartlar| kartlar.len()),
            Ok(16)
        );
    ...
    fn svg_image_wasm_bağımsız_kaynak_belgesini_üretir() {
        ...
        assert!(svg_image_kart_tanim_ornegi().contains("svg_kaydı"));
        assert_eq!(kart_sayisi(), 365);
    }
    ...
    fn log_scales2_wasm_on_iki_kaynak_yüzeyi_üretir() {
        ...
        let Ok(kartlar) = uplot_rs_gpui_ornekler::log_scales2_kartları() else {
            return;
        };
